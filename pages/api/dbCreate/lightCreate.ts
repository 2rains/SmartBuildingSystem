  ...
  fs.readdir("/building_data", (err: any, files: any[]) => {
    ...
    if (err) {
      ...
      try {
            ...
            fs.readdir(
              ...
              async (err: any, files2: any[]) => {
                ...
                for (let j = 0; j < files2.length; j++) {
                  ...
                  const createMany = await client.light.createMany({
                    data: arr_json,
                  });
                }
              }
        ...
        res.status(200).json({ name: "LIGHT OK", arr_json }); // 응답메소드를 여러 번 호출해서 에러남(1번 하면 응답이 끝남)
      } catch (err) {
...
//     const createCo2 = await client.light.create({
//       data: arr_json[i],
//     });
//     console.log(i);
//     i++;
...
//   }, 5000);
//   res.status(200).json({ name: "light OK" });
// } catch (err) {
