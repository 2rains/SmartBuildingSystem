  ...
  fs.readdir("/building_data", (err: any, files: any[]) => {
    ...
    if (err) {
      ...
      try {
            ...
            fs.readdir(
              ...
              async (err: any, files2: any[]) => {
                ...
                for (let j = 0; j < files2.length; j++) {
                  ...
                  const createMany = await client.co2.createMany({
                    data: arr_json,
                  });
                }
              }
        ...
        return res.status(200).json({ name: "CO2 OK" }); // 응답메소드를 여러 번 호출해서 에러남(1번 하면 응답이 끝남)
      } catch (err) {
...
//     const createCo2 = await client.co2.create({
//       data: arr_json[i],
//     });
//     console.log(i);
//     i++;
...
//   }, 5000);
//   res.status(200).json({ name: "co2 OK" });
// } catch (err) {
